...
To get the permissions that you need to manage organization policies, ask your administrator to grant you the following IAM roles:
  ...
  - Create or update AuthProvider configuration: Agent Identity Editor ( `roles/agentidentity.editor` ) or Agent Identity Admin ( `roles/agentidentity.admin` ) on the project resource

For more information about granting roles, see [Manage access to projects, folders, and organizations](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access) .

You might also be able to get the required permissions through [custom roles](https://docs.cloud.google.com/iam/docs/creating-custom-roles) or other [predefined roles](https://docs.cloud.google.com/iam/docs/roles-overview#predefined) .

## Agent Identity supported resources

The following table lists the Agent Identity resources that you can reference in custom constraints.
...
Replace the following:
  ...
  - `  RESOURCE_NAME  ` : the fully qualified name of the Google Cloud resource containing the object and field that you want to restrict. For example, `agentidentity.googleapis.com/AuthProvider` . Most resource types support up to 20 custom constraints. If you attempt to create more custom constraints, the operation fails.
  ...
  - `  CONDITION  ` : a [CEL condition](https://docs.cloud.google.com/resource-manager/docs/organization-policy/creating-managing-custom-constraints#common_expression_language) that is written against a representation of a supported service resource. This field can contain up to 1000 characters. For example, `"has(resource.authProviderTypeParams.threeLeggedOauth) && resource.authProviderTypeParams.threeLeggedOauth.enablePkce == false"` .
...
Replace the following:
  ...
  - `  CONSTRAINT_NAME  ` : the name you defined for your custom constraint. For example, `custom.requirePkceFor3LO` .
...
The following example creates a custom constraint and policy that denies the creation of new 3-legged OAuth authentication providers that don't have PKCE enabled.
...
1.  Save the following file as `constraint-require-pkce.yaml` :
    ...
    This file defines a constraint where for every new authentication provider, if it is a 3-legged OAuth provider and PKCE is not enabled, the operation is denied.
...
3.  Verify that the constraint exists:
    ...
    The output is similar to the following:
        ...
        custom.requirePkceFor3LO    DENY         CREATE,UPDATE  agentidentity.googleapis.com/AuthProvider   Require PKCE for 3LO
        ...

### Create the policy

1.  Save the following file as `policy-require-pkce.yaml` :
...
2.  Apply the policy:
        ...
        gcloud org-policies set-policy policy-require-pkce.yaml
...
3.  Verify that the policy exists:
    ...
    The output is similar to the following:
        ...
        custom.requirePkceFor3LO   -              SET               ...

After you apply the policy, wait for about two minutes for Google Cloud to start enforcing the policy.

### Test the policy

Try to create a 3-legged OAuth provider without enabling PKCE. This command fails if the constraint is enforced:
...
The output is similar to the following:
    ...
    Operation denied by org policy on resource 'projects/PROJECT_ID/locations/global/authProviders/PROVIDER_ID': ["customConstraints/custom.requirePkceFor3LO": "All new 3-legged OAuth authentication providers must have PKCE enabled."]
...
<tbody>
<tr class="odd">
<td>Require description for all AuthProviders</td>
<td><pre dir="ltr" data-is-upgraded="" data-syntax="YAML" translate="no"><code>    name: organizations/ORGANIZATION_ID/customConstraints/custom.requireAuthProviderDescription
    resourceTypes:
    - agentidentity.googleapis.com/AuthProvider
    methodTypes:
    - CREATE
...
<tr class="even">
<td>Disallow 2-legged OAuth AuthProviders</td>
<td><pre dir="ltr" data-is-upgraded="" data-syntax="YAML" translate="no"><code>    name: organizations/ORGANIZATION_ID/customConstraints/custom.disallow2LO
    resourceTypes:
    - agentidentity.googleapis.com/AuthProvider
    methodTypes:
    - CREATE
