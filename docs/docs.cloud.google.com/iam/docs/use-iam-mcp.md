---
name: documents/docs.cloud.google.com/iam/docs/use-iam-mcp
uri: https://docs.cloud.google.com/iam/docs/use-iam-mcp
title: Use the IAM remote MCP server
description: Learn how to use the Identity and Access Management remote MCP server.
data_source: docs.cloud.google.com
---

This document shows you how to use the Identity and Access Management remote Model Context Protocol (MCP) server to connect with AI applications including Gemini CLI, ChatGPT, Claude, and custom applications you are developing. The IAM remote MCP server lets you inspect and manage custom roles and deny policy configurations across your resources. The Identity and Access Management remote MCP server is enabled when you enable the Identity and Access Management API.
...
These predefined roles contain the permissions required to use the Identity and Access Management MCP server. To see the exact permissions that are required, expand the **Required permissions** section:
...
The Identity and Access Management remote MCP server uses the [OAuth 2.0](https://developers.google.com/identity/protocols/oauth2) protocol with [Identity and Access Management (IAM)](https://docs.cloud.google.com/iam/docs/overview) for authentication and authorization. All [Google Cloud identities](https://docs.cloud.google.com/docs/authentication/identity-products) are supported for authentication to MCP servers.

The Identity and Access Management MCP server doesn't support API keys.
...
## Identity and Access Management MCP OAuth scopes
...
Additional scopes might be required on the resources accessed during a tool call. To view a list of scopes required for Identity and Access Management, see [Identity and Access Management API](https://developers.google.com/identity/protocols/oauth2/scopes#iam) .

## Configure an MCP client to use the Identity and Access Management MCP server
...
To view details of available MCP tools and their descriptions for the Identity and Access Management MCP server, see the [Identity and Access Management MCP reference](https://docs.cloud.google.com/iam/docs/reference/mcp) .
...
The IAM remote MCP server lets you manage IAM access configurations across your resources. This includes performing tasks like listing, creating, updating, deleting, and restoring roles, IAM policies, and their bindings.
...
| Create, update, or delete Deny policies in a project.                 | "There is potential data exfiltration in the `confidential-data` project. Create a new deny policy to deny the `suspicious-sa@confidential-data.iam.gserviceaccount.com` service account from all permissions on all BigQuery datasets and Cloud Storage buckets within this project. In the same project, delete the `example_deny` policy."                        |
