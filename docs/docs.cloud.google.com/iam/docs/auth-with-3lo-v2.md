...
You don't construct an auth scheme for this flow. The MCP server's outbound binding already records which auth provider to use. Pass `continue_uri` to specify where to redirect the user after consent.
