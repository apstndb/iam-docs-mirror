---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/workforce-pools/providers/scim-tenants/undelete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/workforce-pools/providers/scim-tenants/undelete
title: gcloud alpha iam workforce-pools providers scim-tenants undelete
...
gcloud alpha iam workforce-pools providers scim-tenants undelete - undelete an IAM workforce identity pool provider SCIM tenant

SYNOPSIS

`gcloud alpha iam workforce-pools providers scim-tenants undelete` ( `  SCIM_TENANT  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(ALPHA)` Undelete a previously deleted SCIM tenant associated with a specific workforce identity pool provider, restoring it to an active state.

EXAMPLES

To undelete a SCIM tenant with ID `my-tenant` under provider `my-okta-provider` in pool `my-pool` located in `global` :

    gcloud alpha iam workforce-pools providers scim-tenants undelete my-tenant --location=global --workforce-pool=my-pool --provider=my-okta-provider
