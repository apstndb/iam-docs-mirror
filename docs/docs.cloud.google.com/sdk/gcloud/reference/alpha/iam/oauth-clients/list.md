---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/oauth-clients/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/oauth-clients/list
title: gcloud alpha iam oauth-clients list
...
gcloud alpha iam oauth-clients list - list OAuth clients

SYNOPSIS

`gcloud alpha iam oauth-clients list` `  --location  ` = `  LOCATION  ` \[ `  --show-deleted  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
The following command lists all OAuth clients in the default project, including the soft-deleted ones:
    ...
    gcloud alpha iam oauth-clients list --location="global" --show-deleted
