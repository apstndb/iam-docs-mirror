---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/roles/undelete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/alpha/iam/roles/undelete
title: gcloud alpha iam roles undelete
...
`gcloud alpha iam roles undelete` `  ROLE_ID  ` ( `  --organization  ` = `  ORGANIZATION  ` | `  --project  ` = `  PROJECT_ID  ` ) \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(ALPHA)` This command undeletes a role. Roles that have been deleted for certain long time can't be undeleted.
...
To undelete the role `  ProjectUpdater  ` of the organization `  1234567  ` , run:
    ...
    gcloud alpha iam roles undelete ProjectUpdater --organization=1234567
