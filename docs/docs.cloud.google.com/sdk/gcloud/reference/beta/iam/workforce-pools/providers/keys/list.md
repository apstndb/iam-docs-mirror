---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/keys/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/keys/list
title: gcloud beta iam workforce-pools providers keys list
...
gcloud beta iam workforce-pools providers keys list - list workforce pool provider keys

SYNOPSIS

`gcloud beta iam workforce-pools providers keys list` ( `  --provider  ` = `  PROVIDER  ` : `  --location  ` = `  LOCATION  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  --show-deleted  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
The following command lists the keys in the workforce pool provider with ID `  my-provider  ` , including soft-deleted keys:
    ...
    gcloud beta iam workforce-pools providers keys list --workforce-pool="my-workforce-pool" --provider="my-provider" --location="global" --show-deleted
