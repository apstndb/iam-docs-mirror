---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/keys/delete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/keys/delete
title: gcloud beta iam workforce-pools providers keys delete
...
gcloud beta iam workforce-pools providers keys delete - delete a workforce pool provider key

SYNOPSIS

`gcloud beta iam workforce-pools providers keys delete` ( `  KEY  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  --async  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(BETA)` Delete a workforce pool provider key.

EXAMPLES

The following command deletes a workforce pool provider key with the ID `  my-key  ` .

    gcloud beta iam workforce-pools providers keys delete my-key --location="global" --workforce-pool="my-workforce-pool" --provider="my-provider"
