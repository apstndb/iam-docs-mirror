---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/scim-tenants/update
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/iam/workforce-pools/providers/scim-tenants/update
title: gcloud beta iam workforce-pools providers scim-tenants update
...
gcloud beta iam workforce-pools providers scim-tenants update - update an IAM workforce identity pool provider SCIM tenant

SYNOPSIS

`gcloud beta iam workforce-pools providers scim-tenants update` ( `  SCIM_TENANT  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  --claim-mapping  ` =\[ `  KEY  ` = `  VALUE  ` , …\]\] \[ `  --description  ` = `  DESCRIPTION  ` \] \[ `  --display-name  ` = `  DISPLAY_NAME  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(BETA)` Update the configuration of an existing SCIM tenant associated with a specific workforce identity pool provider. Only fields specified in the command will be modified.
...
To update the display name and description of a SCIM tenant with ID `my-tenant` under provider `my-okta-provider` in pool `my-pool` located in `global` :
    ...
    gcloud beta iam workforce-pools providers scim-tenants update my-tenant --location=global --workforce-pool=my-pool --provider=my-okta-provider --display-name="Updated Tenant Name" --description="New description"

To update the claim mapping for the same tenant:

    gcloud beta iam workforce-pools providers scim-tenants update my-tenant --location=global --workforce-pool=my-pool --provider=my-okta-provider --claim-mapping="google.subject=new_external_id,google.groups=all_groups"
