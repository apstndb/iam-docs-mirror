---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/beta/iam/policy-bindings/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/beta/iam/policy-bindings/list
title: gcloud beta iam policy-bindings list
...
`gcloud beta iam policy-bindings list` ( `  --location  ` = `  LOCATION  ` : `  --folder  ` = `  FOLDER  ` `  --organization  ` = `  ORGANIZATION  ` ) \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]

DESCRIPTION

`(BETA)` List PolicyBinding instances.

EXAMPLES

To list all policy binding instances in project `my-project` run:

    gcloud beta iam policy-bindings list --project=my-project --location=global

REQUIRED FLAGS

Location resource - The parent resource, which owns the collection of policy bindings.
