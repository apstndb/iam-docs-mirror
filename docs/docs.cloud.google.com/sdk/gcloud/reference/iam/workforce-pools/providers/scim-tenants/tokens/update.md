...
`gcloud iam workforce-pools providers scim-tenants tokens update` ( `  TOKEN  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --scim-tenant  ` = `  SCIM_TENANT  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) `  --display-name  ` = `  DISPLAY_NAME  ` \[ `  GCLOUD_WIDE_FLAG …  ` \]
