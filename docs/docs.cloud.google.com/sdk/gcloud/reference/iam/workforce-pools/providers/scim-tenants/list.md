---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/iam/workforce-pools/providers/scim-tenants/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/iam/workforce-pools/providers/scim-tenants/list
title: gcloud iam workforce-pools providers scim-tenants list
...
`gcloud iam workforce-pools providers scim-tenants list` ( `  --provider  ` = `  PROVIDER  ` : `  --location  ` = `  LOCATION  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  --show-deleted  ` \] \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To list all SCIM tenants under provider `my-okta-provider` in pool `my-pool` located in `global` :
    ...
    gcloud iam workforce-pools providers scim-tenants list --location=global --workforce-pool=my-pool --provider=my-okta-provider

To list deleted SCIM tenants as well:

    gcloud iam workforce-pools providers scim-tenants list --location=global --workforce-pool=my-pool --provider=my-okta-provider --show-deleted
