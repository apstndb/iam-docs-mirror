---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/iam/workforce-pools/providers/scim-tenants/describe
uri: https://docs.cloud.google.com/sdk/gcloud/reference/iam/workforce-pools/providers/scim-tenants/describe
title: gcloud iam workforce-pools providers scim-tenants describe
...
`gcloud iam workforce-pools providers scim-tenants describe` ( `  SCIM_TENANT  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --workforce-pool  ` = `  WORKFORCE_POOL  ` ) \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To describe a SCIM tenant with ID `my-tenant` under provider `my-provider` in pool `my-pool` located in `global` :
    ...
    gcloud iam workforce-pools providers scim-tenants describe my-tenant --location=global --workforce-pool=my-pool --provider=my-provider
