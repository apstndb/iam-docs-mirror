---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/namespaces/undelete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/namespaces/undelete
title: gcloud iam workload-identity-pools namespaces undelete
...
`gcloud iam workload-identity-pools namespaces undelete` ( `  NAMESPACE  ` : `  --location  ` = `  LOCATION  ` `  --workload-identity-pool  ` = `  WORKLOAD_IDENTITY_POOL  ` ) \[ `  --async  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
The following command undeletes a workload identity pool namespace in the default project with the ID `  my-namespace  ` .
...
Workload identity pool namespace resource - The workload identity pool namespace to undelete. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
