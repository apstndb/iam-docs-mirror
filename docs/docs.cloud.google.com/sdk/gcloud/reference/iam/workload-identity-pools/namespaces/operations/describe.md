...
`gcloud iam workload-identity-pools namespaces operations describe` ( `  OPERATION  ` : `  --location  ` = `  LOCATION  ` `  --namespace  ` = `  NAMESPACE  ` `  --workload-identity-pool  ` = `  WORKLOAD_IDENTITY_POOL  ` ) \[ `  GCLOUD_WIDE_FLAG …  ` \]
