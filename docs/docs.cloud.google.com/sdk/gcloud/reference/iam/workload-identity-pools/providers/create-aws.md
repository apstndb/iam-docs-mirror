---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/providers/create-aws
uri: https://docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/providers/create-aws
title: gcloud iam workload-identity-pools providers create-aws
...
`gcloud iam workload-identity-pools providers create-aws` ( `  PROVIDER  ` : `  --location  ` = `  LOCATION  ` `  --workload-identity-pool  ` = `  WORKLOAD_IDENTITY_POOL  ` ) `  --account-id  ` = `  ACCOUNT_ID  ` \[ `  --attribute-condition  ` = `  ATTRIBUTE_CONDITION  ` \] \[ `  --attribute-mapping  ` =\[ `  KEY  ` = `  VALUE  ` , …\]\] \[ `  --description  ` = `  DESCRIPTION  ` \] \[ `  --disabled  ` \] \[ `  --display-name  ` = `  DISPLAY_NAME  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
