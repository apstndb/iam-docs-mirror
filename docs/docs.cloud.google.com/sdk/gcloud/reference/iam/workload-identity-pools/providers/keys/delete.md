---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/providers/keys/delete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/iam/workload-identity-pools/providers/keys/delete
title: gcloud iam workload-identity-pools providers keys delete
...
`gcloud iam workload-identity-pools providers keys delete` ( `  KEY  ` : `  --location  ` = `  LOCATION  ` `  --provider  ` = `  PROVIDER  ` `  --workload-identity-pool  ` = `  WORKLOAD_IDENTITY_POOL  ` ) \[ `  --async  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
The following command deletes a workload identity pool provider key in the default project with the ID `  my-key  ` .
...
Workload identity pool provider key resource - The workload identity pool provider key to delete. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
