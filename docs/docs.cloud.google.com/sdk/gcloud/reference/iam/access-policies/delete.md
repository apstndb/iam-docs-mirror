...
`gcloud iam access-policies delete` ( `  ACCESS_POLICY  ` : `  --folder  ` = `  FOLDER  ` `  --location  ` = `  LOCATION  ` `  --organization  ` = `  ORGANIZATION  ` ) \[ `  --async  ` \] \[ `  --etag  ` = `  ETAG  ` \] \[ `  --force  ` \] \[ `  --validate-only  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
