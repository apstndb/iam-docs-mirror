...
`gcloud iam policy-bindings search-target-policy-bindings` `  --target  ` = `  TARGET  ` ( `  --location  ` = `  LOCATION  ` : `  --folder  ` = `  FOLDER  ` `  --organization  ` = `  ORGANIZATION  ` ) \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  GCLOUD_WIDE_FLAG …  ` \]
